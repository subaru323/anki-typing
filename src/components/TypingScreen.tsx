...
// ── Furigana / skip utilities ─────────────────────────────────────────────

/** Characters that are automatically skipped (no typing required) */
const SKIP_SET = new Set('○●◎◯□■△▲▽▼◇◆☆★※×＊〇〻'.split(''))

interface Segment {
  text: string       // displayed text (may be multi-char for ruby groups)
  ruby?: string      // furigana reading
  skip: boolean      // auto-skip, no typing needed
}

/** Parse {漢字|よみ} notation into Segment array */
function parseSentence(raw: string): Segment[] {
  const segs: Segment[] = []
  let last = 0
  const re = /\{([^|{}]+)\|([^|{}]+)\}/g
  ...
  while ((m = re.exec(raw)) !== null) {
    for (const ch of raw.slice(last, m.index)) {
      segs.push({ text: ch, skip: SKIP_SET.has(ch) })
    }
    segs.push({ text: m[1], ruby: m[2], skip: false })
...
/** Build the string the user must type (skip chars removed) */
function buildTarget(segs: Segment[]): string {
  return segs.filter(s => !s.skip).map(s => s.text).join('')
}
...
export default function TypingScreen({ sentences, timeLimit, onBack }: Props) {
  ...
  /** true while IME is composing (romaji → kana → kanji) */
  const isComposingRef = useRef(false)

  // Derived values — recalculate whenever currentRaw changes
  const segments = useMemo(() => parseSentence(currentRaw), [currentRaw])
  const typingTarget = useMemo(() => buildTarget(segments), [segments])

  // Keep a stable ref of typingTarget for use inside effects
  ...
  useEffect(() => {
    ...
    if (timeLeft <= 0) {
      // Time's up — if mid-sentence-done, still count it
      if (phase === 'sentence-done') {
        setCompletedChars(prev => prev + typingTargetRef.current.length)
  ...
  function handleCompositionEnd() {
    // compositionEnd fires just before the final onChange
    isComposingRef.current = false
  }

  // ── Typing ───────────────────────────────────────────────────────────────
  function handleChange(e: React.ChangeEvent<HTMLInputElement>) {
    // Ignore changes during IME composition (mid-conversion state)
    if (isComposingRef.current) return
    if (phase !== 'playing') return

    const value = e.target.value

    // Count errors on each newly added character
    if (value.length > typed.length) {
      const idx = value.length - 1
      if (idx < typingTarget.length && value[idx] !== typingTarget[idx]) {
    ...
    // Sentence complete?
    if (value === typingTarget && typingTarget.length > 0) {
      setPhase('sentence-done')
    }
  }

  // ── Keyboard (Enter = advance after sentence, or IME confirm) ────────────
  ...
  // ── Sentence display with char-level highlight ────────────────────────────
  function renderSentence() {
    const isDone = phase === 'sentence-done'
    let targetIdx = 0   // cursor in typingTarget

    return segments.map((seg, si) => {
      if (seg.skip) {
        return (
          <span key={si} style={{ color: '#3a3a5a', textDecoration: 'line-through', opacity: 0.6 }}>
            {seg.text}
          </span>
        )
      }
      ...
      const segStart = targetIdx
      targetIdx += segLen

      // Per-character status for each char in this segment
      const chars = seg.text.split('').map((ch, ci) => {
        const pos = segStart + ci
        let color = '#4a4a6a'
        let bg = 'transparent'

        if (isDone) {
          // All chars turn green on completion
          color = '#4ade80'
        } else if (pos < typed.length) {
          color = typed[pos] === ch ? '#4ade80' : '#f87171'
          bg   = typed[pos] !== ch ? 'rgba(248,113,113,0.1)' : 'transparent'
        } else if (pos === typed.length) {
          color = '#e2e8f0'
        ...
        return (
          <span key={ci} style={{ color, background: bg, borderRadius: '3px', transition: 'color 0.06s' }}>
            {ch}
          </span>
        )
      })
      ...
      if (seg.ruby) {
        return (
          <ruby key={si} style={{ margin: '0 1px' }}>
            {chars}
            <rt style={{ fontSize: '0.52em', color: '#7c6af7', letterSpacing: '0.05em' }}>
              {seg.ruby}
            </rt>
          </ruby>
        )
  ...
  if (phase === 'finished') {
    ...
    const rankColor = { S: '#fbbf24', A: '#4ade80', B: '#7c6af7', C: '#64748b', D: '#f87171' }[rank]
    return (
      <div className="min-h-screen flex flex-col items-center justify-center px-6" style={{ background: '#0d0d1a' }}>
        <div className="w-full max-w-md rounded-2xl px-10 py-12 text-center"
          ...
          <div className="flex gap-3">
            <button onClick={onBack} className="flex-1 py-3 rounded-lg font-bold text-sm tracking-widest cursor-pointer"
            ...
            <button onClick={resetGame} className="flex-1 py-3 rounded-lg font-bold text-sm tracking-widest cursor-pointer"
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ background: '#0d0d1a' }} onClick={focusInput}>
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-8 py-10">
        ...
        {/* Sentence display */}
        <div className="w-full max-w-3xl mb-10" style={{ minHeight: '4rem' }}>
        ...
        <div className="w-full max-w-2xl relative">
          <input
            ...
            style={{
              background: isDone ? 'rgba(74,222,128,0.05)' : '#12122a',
        ...
        {isDone ? (
          ...
          <>
            <div className="w-full max-w-2xl mt-4 rounded-full overflow-hidden" style={{ height: '3px', background: '#1a1a3a' }}>
              <div style={{
                width: `${typingTarget.length > 0 ? (typed.length / typingTarget.length) * 100 : 0}%`,
                height: '100%',
            ...
            <div className="mt-3 text-xs" style={{ color: '#3a3a5a', fontFamily: 'Courier New, monospace' }}>
              {typed.length} / {typingTarget.length} 文字
            </div>
          </>
        )}
      </div>
